...
const PayroleTable = () => {
    ...
    useEffect(() => {
        if (!allKaidi || allKaidi.length === 0) {
            setFilteredKaidi([]);
            return;
        }

        const noFiltersApplied =
            (searchOffice === '' || searchOffice === null || searchOffice === undefined) &&
            (searchpayroleStatus === '' || searchpayroleStatus === null || searchpayroleStatus === undefined || searchpayroleStatus === '0' || searchpayroleStatus === 0) &&
            !searchpyarole_rakhan_upayukat &&
            !searchpayrole_no_id &&
            !searchmudda_id &&
            !searchbandi_name &&
            (searchis_checked === '' || searchis_checked === null || searchis_checked === undefined);

        if (noFiltersApplied) {
            console.log(noFiltersApplied)
            setFilteredKaidi(allKaidi);
        ...
        if (searchOffice) {
            filtered = filtered.filter(a => String(a.created_office) === String(searchOffice));
        }
        console.log(searchpayroleStatus)
        if (searchpayroleStatus !== '' && searchpayroleStatus !== null && searchpayroleStatus !== undefined) {
            const status = parseInt(searchpayroleStatus);
            filtered = filtered.filter(a => a.payrole_status === status);
        }
        ...
        if (searchpayrole_no_id) {
            filtered = filtered.filter(a => String(a.payrole_no_id) === String(searchpayrole_no_id));
        }

        if (searchmudda_id) {
            filtered = filtered.filter(a => String(a.mudda_id) === String(searchmudda_id));
        }

        if (searchbandi_name) {
            filtered = filtered.filter(a =>
                a.bandi_name && a.bandi_name.toLowerCase().includes(searchbandi_name.toLowerCase())
            );
        }

        if (searchis_checked !== '' && searchis_checked !== null && searchis_checked !== undefined) {
            const chk = parseInt(searchis_checked);
            filtered = filtered.filter(a => a.is_checked === chk);
        }
    ...
    const handleChangePayroleStatus = (data, value) => {
        const pesh = async () => {
            try {
                await axios.put(`${BASE_URL}/bandi/update_payrole_status/${data.payrole_id}`, { value }, { withCredentials: true });
                fetchKaidi();
                // Optionally update local state or re-fetch
            } catch (err) {
                console.error('Update failed:', err);
    ...
    const handleKaragarStatusChange = async (updatedData) => {
        console.log(updatedData)
        try {
            await axios.put(
                `${BASE_URL}/bandi/update_payrole_decision/${updatedData.payrole_id}`,
                updatedData,
                                            ...
                                            <TableRow sx={rowStyle}>
                                                ...
                                                <TableCell rowSpan={kaidiMuddas.length || 1}>
                                                    {data.payrole_status === 1 ? (
                                                        <Button variant="contained" color="primary" onClick={() => handleChangePayroleStatus(data, 1)}>
                                                            पेश गर्नुहोस्
                                                        </Button>
