...
const ReusePayroleStatus = ({ name, label, required, readonly, control, error, defaultvalue }) => {
    ...
    const fetchPayroleStatus = async () => {
        try {
            const url = `${BASE_URL}/public/get_payrole_status`;
            ...
            if (Status) {
                if (Array.isArray(Result) && Result.length > 0) {
                    const formatted = Result.map((opt) => ({
                        label: opt.payrole_status_name, // Use Nepali name
                        value: opt.id, // Use ID as value
                    }));
                    setFormattedOptions(formatted);
                } else {
                    console.log('No payrole status found.');
                }
            } else {
                console.log(Error || 'Failed to fetch payrole status.');
            }
        } catch (error) {
            console.error('Error fetching payrole status:', error);
        }
    };
...
export default ReusePayroleStatus;
