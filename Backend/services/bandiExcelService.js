...
export const generateBandiExcelWithPhoto = async ( job, filters ) => {
    ...
    for ( const r of rows ) {
        // Suppose r.muddas is an array of mudda objects for this bandi
        const muddas = r.muddas && r.muddas.length ? r.muddas : [{}];
        const bandiRows = [];

        for ( let i = 0; i < muddas.length; i++ ) {
            const m = muddas[i];
        ...
        // Merge bandi info columns for multi-mudda rows
        ...
        if ( r.photo_path ) {
            ...
            if ( fs.existsSync( photoFullPath ) ) {
                ...
                sheet.addImage( imageId, {
                    tl: { col: headers.length - 1, row: bandiRows[0].number - 1 },
