CREATE
OR REPLACE VIEW view_bandi_full AS
SELECT
    b.id AS bandi_id,
    b.office_bandi_id,
    b.lagat_no,
...
FROM
    ...
    LEFT JOIN (
        ...
        WHERE
            relation_id = 1
    ) AS bSpouse ON b.id = bSpouse.bandi_id
    LEFT JOIN (
        SELECT
            bandi_id,
            relative_name,
        ...
        WHERE
            relation_id = 2
    ) AS bFather ON b.id = bFather.bandi_id
    LEFT JOIN (
        SELECT
            bandi_id,
            relative_name,
        ...
        WHERE
            relation_id = 3
    ) AS bMother ON b.id = bMother.bandi_id
    LEFT JOIN (
        SELECT
            bcp.bandi_id,
            GROUP_CONCAT (
                ...
                ORDER BY
                    rt.relation_np SEPARATOR ' | '
            ) AS other_relatives
        FROM
            bandi_contact_person bcp
