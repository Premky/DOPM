...
router.get( '/get_all_office_bandi', verifyToken, async ( req, res ) => {
    ...
    if ( is_escape === 'escaped' ) {
        ...
        conditions.push( `(bed.status = "recaptured") AND (bed.escaped_from_office_id=${active_office} OR bed.current_office_id=${active_office})`);
    } else if ( is_escape === 'self_present' ) {
        conditions.push( `(bed.status = "self_present") AND (bed.escaped_from_office_id=${active_office} OR bed.current_office_id=${active_office})`);
    }
