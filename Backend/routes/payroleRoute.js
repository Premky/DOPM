...
router.get( "/summary/mudda_wise", paroleController.getParoleSummary );
router.get( "/summary/mudda_wise/export", paroleController.exportParoleSummary );

router.get( "/summary/office_wise", paroleController.getParoleSummary );
router.get( "/summary/office_wise/export", paroleController.exportParoleSummary );
...
router.get( '/get_payrole_logs/:id', verifyToken, async ( req, res ) => {
    ...
    `;
        ...
        if ( user_office_id >= 2 ) {
            finalQuery += ` AND (p.created_office = '?' OR p.updated_office = '?')`;
            queryParams = [...queryParams, user_office_id, user_office_id];
        }
        const [result] = await pool.query( finalQuery, queryParams );
        // console.log( 'acceptedpayrole', user_office_id );
        if ( !result.length ) {
...
router.get( '/get_bandi_parole/:id', async ( req, res ) => {
    ...
    const sql = `
        ...
        WHERE p.bandi_id = ? AND p.STATUS IN(1,2,3,4,7)
    `;
    try {
        const [result] = await pool.query( sql, [id] ); // Use promise-wrapped query        
        if ( result.length === 0 ) {
...
router.get( '/get_allowed_actions', verifyToken, async ( req, res ) => {
    const active_office = req.user.office_id;
    const user_role_id = req.user.role_id;
    const payrole_status_id = Number( req.query.payrole_status_id );
    console.log( 'payrole_status_id', payrole_status_id, 'user_role_id', user_role_id );
    try {
        const [result] = await pool.query( `
